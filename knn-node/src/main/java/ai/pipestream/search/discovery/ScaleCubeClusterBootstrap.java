...
public class ScaleCubeClusterBootstrap {
    ...
    void onStart(@Observes io.quarkus.runtime.StartupEvent ev) {
        ...
        try {
            ...
            String bindHost = ConfigProvider.getConfig()
                    .getOptionalValue("knn.scalecube.host", String.class).orElse(null);

            ClusterConfig config = ClusterConfig.defaultLanConfig()
                    .externalHost(bindHost)
            ...
            boolean separateGrpcServer = ConfigProvider.getConfig()
                    .getOptionalValue("quarkus.grpc.server.use-separate-server", Boolean.class)
                    .orElse(false);
            int grpcPort = separateGrpcServer
                    ? ConfigProvider.getConfig().getOptionalValue("quarkus.grpc.server.port", Integer.class).orElse(48101)
                    : ConfigProvider.getConfig().getOptionalValue("quarkus.http.port", Integer.class).orElse(48100);
