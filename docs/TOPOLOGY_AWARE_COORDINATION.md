...
*   **Graph Skew**: Different shards have different connectivity. A 0.88 on the Ryzen host might be "deep" in its graph, while another node at the same timestamp is still "shallow" and finding 0.80s.
...
*   **Payload**: 128 bytes.
*   **Pros**: No pre-indexing requirement; low-overhead bit operations (XOR/Popcount) are computationally cheap.
